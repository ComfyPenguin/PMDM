...
class LlistaEntrants extends StatelessWidget {
  final List<Entrant> llistaEntrants;
  const LlistaEntrants(this.llistaEntrants, {super.key});
  ...
  Widget build(BuildContext context) {
    return ListView.builder(
      ...
      itemBuilder: (BuildContext context, int index) {
        return ListItem(
          nom: llistaEntrants[index].name,
          img: llistaEntrants[index].img ?? "",
          preu: llistaEntrants[index].price ?? 0.0,
          info: llistaEntrants[index].additionalInfo ?? "",
        );
      },
    );
  }
