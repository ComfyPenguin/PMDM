...
class LlistaPrincipals extends StatelessWidget {
  final List<Principal> llistaPrincipals;
  const LlistaPrincipals(this.llistaPrincipals, {super.key});
  ...
  Widget build(BuildContext context) {
    return ListView.builder(
      ...
      itemBuilder: (BuildContext context, int index) {
        return ListItem(
          nom: llistaPrincipals[index].name,
          img: llistaPrincipals[index].img ?? "",
          preu: llistaPrincipals[index].price ?? 0.0,
          info: llistaPrincipals[index].additionalInfo ?? "",
        );
      },
    );
  }
